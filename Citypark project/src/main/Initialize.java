	...
	private static void afrekenen(int pas_id, String Card_ID, double bedrag){
		...
		if((Card_ID.equals("STX EED6326ACR LF ") || Card_ID.equals("STX D4F9374CCR LF ")) && bedrag > 0.00){
			try{
				...
				databaseCitypark.update("UPDATE inrijden SET eindtijd='"+formattedDate+"', betaald = 1 WHERE Pas_Pas_ID='"+pas_id2+"';");
				
				PinView pinview = new PinView(rekeningnmr, bedrag);
				while(!pinview.getBetaald()){
				}
				poortOpenen();				
			}catch(Exception e){
				e.getStackTrace();
